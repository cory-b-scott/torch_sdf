...
from torch_sdf.ops.binary_ops import nary_smooth_union_exp
...
class MultiCircleSDF(torch.nn.Module):
    ...
    def forward(self, query):
        ...
        return nary_smooth_union_exp(dists, k=self.k)

def get_image_coords(K):
    scale = (1/(K-1))
    t = scale*(np.mgrid[:K, :K])
    print(t.shape)
...
if __name__ == '__main__':
    ...
    model = MultiCircleSDF(3, device=dev)
